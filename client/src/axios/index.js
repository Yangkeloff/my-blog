...
export default {
  // 获取标签列表
  api_get_tags(){
    return instance.get("/api/client/get_tags")
  },
  // 获取作品列表
  api_get_work(){
  ...
  api_get_article_list(data){
    ...
    return instance.get(`/api/client/article_list?current_page=${current_page}&page_size=${page_size}&keyword=${keyword}&tag=${tag}&state=${state}`)
  },
  // 通过文章id来获取文章
  api_get_article(id){
    return instance.get(`/api/client/article/${id}`)
  },
  // 获取主要评论
  api_get_comment(data){
    return instance.get(`/api/client/comment?current_page=${data.current_page}&page_size=${data.page_size}`)
  },
  // 根据文章id获取评论
  api_get_comment_by_article(id){
    return instance.get(`/api/client/comment/${id}`)
  },
  // 获取设置
  api_get_setting(){
