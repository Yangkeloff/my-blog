...
const {JWT_ADD_STR,URL_YES_PASS} = require('./config')
...
module.exports = {
  ...
  async check_token(ctx, next){
    ...
    if(ctx.method != 'GET' || !URL_YES_PASS.includes(url)){
      ...
      try {
        // 验证token是否过期
        let {admin_name = ""} = await jwt.verify(token, JWT_ADD_STR)
        // 验证token与账号是否匹配
        let res = await Admin.find({admin_name,token})
        if(res.length == 0){
